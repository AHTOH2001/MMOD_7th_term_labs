...
class DiscreteSVGenerator:
    def __init__(self, matrix, x_values, y_values):
        ...
        self.row_cumsums = np.cumsum(matrix, axis=1) / self.row_partial_sums.reshape(
            -1, 1
        )
...
matrix = np.array(
    [
        ...
        [
            0.0368898846050644,
            0.33786296642833963,
            0.011532421452695715,
            0.059387608341158686,
        ],
        [
            0.034094261530760894,
            0.054284823798560575,
            0.04244115008519647,
            0.008393121304567371,
        ],
    ]
)
x_values = np.array([4.0, 5.0, 9.0])
y_values = np.array([2.0, 4.0, 6.0, 8.0])


def compute_chi_square_statistics(
    theoretical_probability_matrix: np.array,
    emperical_probability_matrix: np.array,
    samples_amount: int = 1000,
):

    return samples_amount * np.sum(
        (emperical_probability_matrix - theoretical_probability_matrix) ** 2
        / theoretical_probability_matrix
    )


def chi_square_test(
    theoretical_probability_matrix: np.array,
    emperical_probability_matrix,
    samples_amount: int,
    confidence_level=0.05,
):
    chi_square_statistics = compute_chi_square_statistics(
        theoretical_probability_matrix, emperical_probability_matrix, samples_amount
    )

    p_value = 1 - sta.chi2.cdf(
        chi_square_statistics, theoretical_probability_matrix.size - 1
    )
...
def calculate_theoretical_and_empirical_expected_values(
    matrix, x_values, y_values, amount=1000
):
    empirical_matrix = empiric_probability_matrix(matrix, x_values, y_values, amount)
    theoretical = expected_values(matrix, x_values, y_values)
    empirical = expected_values(empirical_matrix, x_values, y_values)
...
def calculate_intervals_expectation(values, n=1000, confidence_level=0.95):
    ...
    return (
        sample_mean - np.sqrt(sample_var / n) * normal_quantile,
        sample_mean + np.sqrt(sample_var / n) * normal_quantile,
    )
...
def interval_for_x_and_y(matrix, x_values, y_values, amount=1000):
    ...
    print(
        'Доверительный интервал для оценки математического ожидания X: ', interval_for_x
    )

    interval_for_y = calculate_intervals_expectation(y_samples, amount)
    print(
        'Доверительный интервал для оценки математического ожидания Y: ', interval_for_y
    )
...
def variance(matrix, x_values, y_values):
    ...
    return (
        np.square(x_values) @ np.sum(matrix, axis=1) - m_x**2,
        np.square(y_values) @ np.sum(matrix, axis=0) - m_y**2,
    )


def calculate_theoretical_and_empirical_variance(
    matrix, x_values, y_values, amount=1000
):
    empirical_matrix = empiric_probability_matrix(matrix, x_values, y_values, amount)
    theoretical, empirical = variance(matrix, x_values, y_values), variance(
        empirical_matrix, x_values, y_values
    )
...
def covariation(matrix, x_values, y_values):
    return x_values @ matrix @ y_values - np.prod(
        expected_values(matrix, x_values, y_values)
    )
...
def correlation_coeff(matrix, x_values, y_values, amount=1000):
    ...
    return covariation(matrix, x_values, y_values) / np.sqrt(
        np.prod((x_samples, y_samples))
    )
...
def calculate_empiric_and_theoretic(matrix, x, y, amount=1000):
    ...
    print(
        'Теоретический коэффициент корреляция = ',
        correlation_coeff(matrix, x_values, y_values),
    )
    print(
        'Эмпирический коэффициент корреляция = ',
        correlation_coeff(empirical_matrix, x_values, y_values),
    )
